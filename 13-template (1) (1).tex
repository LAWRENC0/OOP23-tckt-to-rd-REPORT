...
\title{OOP PROGETTO\\``TICKET TO RIDE''}

\author{TICKET TO RIDEZ}
\date{\today}
...
\section{Requisiti}
L'applicazione vuole emulare il gioco da tavolo "Ticket to Ride", con qualche modifica al regolamento. Il progetto si propone di realizzare il gioco da tavolo Ticket to Ride, in cui i giocatori raccolgono Carte Treno di diversi colori per collegare città su una mappa. L'obiettivo è completare rotte specifiche assegnate tramite Carte Obiettivo segrete, cercando di realizzare la rete ferroviaria più lunga e collegare città specifiche per ottenere punti bonus. Il vincitore è colui che accumula più punti alla fine del gioco. 

\subsubsection{Requisiti funzionali}
Il numero di giocatori sarà variabile (da 2 a 6), giocheranno tutti dalla stessa istanza dell'applicazione alernandosi. 
La partita è suddivisa in varie fasi:
\begin{itemize}
\item Preparazione partita: i giocatori pescano tre carte e ne scelgono una, questa azione si ripete due volte per ogni giocatore. Le carte pescate sono Carte Obiettivo, ovvero indicano una tratta da completare piazzando treni. Ogni giocatore riceve anche un certo numero di Carte Treno e Carrozze.
\item Fase di gioco: ad ogni turno il giocatore pesca dal mazzo di Carte Treno e può scegliere se riempire una Tratta. Se così facendo completa una delle sue Carte Obiettivo, allora necessariamente estrae 3 Carte Obiettivo delle quali ne sceglie una.
\item Terminazione del gioco: la partita termina allo scadere del primo turno in cui almeno un giocatore ha un numero di Carrozze inferiore alla lunghezza della tratta più corta rimanente sulla Plancia. 
\item Resoconto partita: a termine del gioco viene visualizzata una scoreboard con i punteggi dei singoli giocatori.
\end{itemize}

\subsubsection{Requisiti non funzionali}
\begin{itemize}
	\item L'esperienza di gioco dovrà essere fluida, gradevole e divertente!! :D
...
Questo progetto ha l'obiettivo di simulare un match di "Ticket to Ride" tra 2<=n<=6 entità Giocatore. Il gioco prevede l'interazione da diverse entità Giocatore, ognuna dei quali ha a disposizione entità Carte Treno, Carte Obiettivo (messe a disposizione da entità Mazzo), e Carrozze (distribuite a inizio partita). I Giocatori vedono l'entità GameBoard, che si compone di entità Città e Tratta (le Tratte collegano tra loro le Città). I Giocatori possono cambiare lo stato delle Tratte sfruttando le proprio carte Treno. (Completando Tratte specifiche vengono completate Carte Obiettivo). Lo stato di gioco è stabilito alla fine del turno di ogni giocatore da un entità GameState. A partita terminata viene stilata la ScoreBoard a seguito del calcolo dei punteggi.
                                        NOTA: EVENTUALI DIFFICOLTA'
...
\begin{center}
\includegraphics[scale=0.7]{interfacciapunteggiUML.png}
\end{center}
...
\begin{center}
\includegraphics[scale=0.7]{objectivegeneratorUML.png}
\end{center}
...
\begin{center}
\includegraphics[scale=0.7]{numerodigiocatoriUML.png}
\end{center}
...
\begin{itemize}
 \item Utilizzo di JavaFX: una libreria mai usata e non vista a lezione. Sopratutto agli inzi non é stato facile capire quale componente fosse piú adatto alle mie necessitá e capire come posizionare gli elementi nello spazio.
